export default async function handler(prisma: any, ward: string, district: string|false, city: string|false) {
    ...
    if(!district && !city) 
    {
        ...
        select ward, district, city, st_asgeojson(ST_Boundary(st_transform(ward_way,4326))), st_x(st_transform(st_centroid(ward_way),4326)), st_y(st_transform(st_centroid(ward_way),4326)) from wards_forsearch
